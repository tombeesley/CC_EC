...
% set which repeated/random patterns get arrows
if rem(subNum,2) == 1
    % repeated Top Left/Bottom Right (col 1)
    % random Top Right/Bottom Left (col 2)
    arrowPats = [1 0 0 1; 0 1 1 0]; 
else
    % opposite asignment
    arrowPats = [0 1 1 0; 1 0 0 1];
end
for trial = 1:size(TT,1)
    ...
    arrowPresented = 0;
    if trial < Stage2Start && arrowPats(TT(trial,4),TT(trial,6)) == 1
        arrowPresented = 1;
            if tLoc <= 72
...
%     Screen('DrawLine', MainWindow, [255 0 0], 960, 0, 960, 1080, 1); % red gridlines
    ...
    DATA.trial_data(trial,2:7) = TT(trial,:);
    DATA.trial_data(trial,8) = targetLoc;
    DATA.trial_data(trial,9) = Torient(orientCnt);
    DATA.trial_data(trial,10) = arrowPresented;
    DATA.trial_data(trial,11) = choice;
    DATA.trial_data(trial,12) = accuracy;
    DATA.trial_data(trial,13) = RT; 
