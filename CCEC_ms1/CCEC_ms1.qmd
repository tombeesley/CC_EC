...
To compare the size of the CC effect across phases 1 and 2, we calculated a "CC effect score" by subtracting the RT on consistent repeated trials from the RT on consistent random trials. Positive values reflect a CC effect. There was a CC effect score of `r spec_dec(CC_transition_summary[1,2],2)` ms (SD = `r spec_dec(CC_transition_summary[1,3],2)`) for the end of phase 1 (epochs 3-5) and a CC effect score of `r spec_dec(CC_transition_summary[2,2],2)` ms (SD = `r spec_dec(CC_transition_summary[2,3],2)`) for the start of phase 2 (epochs 6-8). A Bayesian t-test of the effect of phase on CC effect found moderate support for the null result, BF = `r as.numeric(as.vector(bf_transition_exp1))`, suggesting that the CC effect was not attenuated in the second phase.
