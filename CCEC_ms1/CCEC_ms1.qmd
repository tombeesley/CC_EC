...
# work out CC effect for con trials across blocks
by_subj_CCeffect_transition_e1 <- 
  data_e1_f %>%
  filter(str_detect(TT, pattern = "inconsistent", negate = TRUE)) %>% 
  group_by(subj, TT, epoch) %>%
  summarise(meanRT = mean(RT)) %>%
  pivot_wider(names_from = TT, values_from = meanRT) %>% 
  rename(rep = "Repeated: consistent",
         rand = "Random: consistent") %>% 
  mutate(CC_effect = rand-rep)

# plot of the CC effect across blocks
# by_subj_CCeffect_transition_e1 %>% 
#   group_by(epoch) %>% 
#   summarise(meanCC = mean(CC_effect),
#             seCC = sd(CC_effect)/sqrt(n())) %>% 
#   ungroup() %>% 
#   ggplot(aes(x = epoch, y = meanCC, group = 1)) +
#   geom_line()

# avg CC effect before and after transition
by_subj_CCeffect_transition_e1 %>%
  filter(epoch %in% c(4:7)) %>% 
  pivot_wider(names_from = epoch, values_from = CC_effect)
