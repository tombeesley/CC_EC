...
0.96\%. The addition of the interaction term did not improve the model
fit, BF = 0.47 ± 4.05\%, though there was no evidence for the absence of
the interaction. The best fitting model was a better fit than the two
models containing only one of the factors, smallest BF = 35.8 ± 1\%,
...
BF\textsubscript{10} = 47.02 ± 1.12\%. The next best fitting model
contained these two factors and the interaction term, and was not a
substantially worse fit to the data, BF = 0.77 ± 30.57\%. The best
...
containing only the factor of configuration BF = 20.6 ± 1.5\% providing
...
and the model containing only the factor of target position, BF = 2.41 ±
1.29\%. The relevant effect sizes (\(n^2_p\)) were: 0.14 for the effect
...
BF\textsubscript{10} = 7.5 × 10\textsuperscript{100} ± 1.86\%. The next
best fitting model contained all three factors and the interaction of
epoch and configuration, BF\textsubscript{10} = 5.4 ×
10\textsuperscript{100} ± 3.01\%, and this model was not a substantially
worse fit to the data, BF = 0.72 ± 3.54\%. All other models were
substantially worse fits than the best fitting model, largest BF = 0.26
± 5.01\%. Importantly, the interaction term between the factors of
...
0.19 ± 4.42\%. The relevant effect sizes (\(n^2_p\)) were: 0.02 for the
...
10\textsuperscript{14} ± 0.95\%. There was substantial support for this
model over the next best fitting model, BF = 8.98 ± 1.74\%. To examine
...
interaction, BF = 0.12 ± 5.59\%. The relevant effect sizes (\(n^2_p\))
...
BF\textsubscript{10} = 3.6 × 10\textsuperscript{51} ± 0.91\%. The
...
1.83\%. The relevant effect sizes (\(n^2_p\)) were: 0.77 for the effect
...
10\textsuperscript{24} ± 1.01\%. However, the model without the
interaction provided a strong fit to the data, BF\textsubscript{10} =
5.1 × 10\textsuperscript{24} ± 0.59\%, and a comparison between the two
models did not find any evidence in support of the interaction term, BF
= 0.95 ± 1.17\%. There was strong support for the best fitting model
over the remaining models, smallest BF = 3918.61 ± 1.04\%, providing
...
BF\textsubscript{10} = 1.4 × 10\textsuperscript{14} ± 0.74\%. This model
provided a superior fit to the data compared to the next best fitting
model that included the two factors and the interaction term, BF = 122.4
± 1.16\%, providing strong support for the contribution of the two
...
\url{http://github.com/tombeesley/CC_EC}. The analyses reported in this
manuscript are computationally reproducible from the manuscript source
files (using R v4.4.0), which are available at the github repository.
The study designs and analyses were not pre-registered.
