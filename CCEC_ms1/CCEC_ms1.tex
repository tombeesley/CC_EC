...
the presence of the endogenous cue did not impede the acquisition of
...
guide attention towards the target. This suggests that the early part of
visual search is inconsequential for the development and maintenance of
the contextual cuing effect, and that memory representations are
flexibly deployed when the search procedure is dramatically
interrupted.\\
...
and Chun (\citeproc{ref-olson2002}{2002}), where participants were
...
A recent study by Seitz et al. (\citeproc{ref-seitz2023}{2023}) has
...
Importantly, Seitz et al. (\citeproc{ref-seitz2023}{2023}) suggest that
...
(\citeproc{ref-r_lang}{2024})). All analyses in this study used the
...
point'' (\citeproc{ref-rouder2017}{Rouder et al., 2017, p. 317}). First
...
vs.~random), BF\textsubscript{10} = 2.4 × 10\textsuperscript{12} ±
9.77\%. The addition of the interaction term did not improve the model
fit, BF = 0.4 ± 9.82\%, though there was no evidence for the absence of
the interaction. The best fitting model was a better fit than the two
models containing only one of the factors, smallest BF = 40.25 ± 9.78\%,
...
BF\textsubscript{10} = 46.96 ± 0.99\%. The next best fitting model
contained these two factors and the interaction term, and was not a
substantially worse fit to the data, BF = 0.65 ± 16.09\%. The best
...
containing only the factor of configuration BF = 20.99 ± 1.2\% providing
...
and the model containing only the factor of target position, BF = 2.36 ±
1.35\%. The relevant effect sizes (\(n^2_p\)) were: 0.14 for the effect
...
been suggested to occur in CC (\citeproc{ref-seitz2023}{Seitz et al.,
2023}). According to this account, ``\ldots what may look an ineffective
...
functional.'' (\citeproc{ref-seitz2023}{Seitz et al., 2023, p. 9}). The
...
BF\textsubscript{10} = 7.3 × 10\textsuperscript{100} ± 2.45\%. The next
best fitting model contained all three factors and the interaction of
epoch and configuration, BF\textsubscript{10} = 5.2 ×
10\textsuperscript{100} ± 1.2\%, and this model was not a substantially
worse fit to the data, BF = 0.71 ± 2.73\%. All other models were
substantially worse fits than the best fitting model, largest BF = 0.26
± 3.25\%. Importantly, the interaction term between the factors of
...
0.19 ± 3.89\%. The relevant effect sizes (\(n^2_p\)) were: 0.44 for the
...
factors, BF\textsubscript{10} = 9.5 × 10\textsuperscript{14} ± 0.68\%.
There was substantial support for this model over the next best fitting
model, BF = 8.74 ± 1.47\%. To examine the interaction of the
...
3.02\%. The relevant effect sizes (\(n^2_p\)) were: 0.62 for the effect
...
BF\textsubscript{10} = 3.6 × 10\textsuperscript{51} ± 0.92\%. The
...
1.92\%. The relevant effect sizes (\(n^2_p\)) were: 0.77 for the effect
...
between those factors, BF\textsubscript{10} = 5.5 ×
10\textsuperscript{24} ± 1.43\%. However, the model without the
interaction provided a strong fit to the data, BF\textsubscript{10} =
5.2 × 10\textsuperscript{24} ± 1.03\%, and a comparison between the two
models did not find any evidence in support of the interaction term, BF
= 0.95 ± 1.76\%. There was strong support for the best fitting model
over the remaining models, smallest BF = 3981.25 ± 1.45\%, providing
...
BF\textsubscript{10} = 1.4 × 10\textsuperscript{14} ± 0.53\%. This model
...
123.09 ± 1.2\%, providing strong support for the contribution of the two
...
the repeated configuration (\citeproc{ref-beesley2018}{Beesley et al.,
...
the formation of visual associations (\citeproc{ref-renaux2017}{Renaux
et al., 2017}). Alternatively, it provides support for the proposed
...
(\citeproc{ref-brady2007}{Brady \& Chun, 2007}) and empirical
(\citeproc{ref-olson2002}{Olson \& Chun, 2002}) work that has
...
by Seitz et al. (\citeproc{ref-seitz2023}{2023}), it's at least possible
...
(\citeproc{ref-chen2022}{Chen et al., 2022}). In the present tasks, such
...
\bibitem[\citeproctext]{ref-chen2022}
...
\bibitem[\citeproctext]{ref-r_lang}
