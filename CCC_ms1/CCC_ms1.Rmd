...
rt_summary_data_e1 %>% 
  ...
  geom_errorbar(aes(ymin = group_RT - group_SE,
                ...
                width = .2,
                size = .5) +
  geom_point(aes(fill = TT),size = 8, shape = 21) +
  labs(y = "RT (ms)",
...
data_exp3 <- 
  data_exp3 %>% 
  mutate(TT = fct_recode(TT, 
                         "Repeated GLOBAL (local random)" = "global (local random)",
                         "Repeated LOCAL (global random)" = "local (global random)",
                         "Random" = "random",
                         "Repeated" = "repeated")) %>% 
  mutate(TT = fct_relevel(TT, 
                          "Repeated", "Random", "Repeated GLOBAL (local random)", "Repeated LOCAL (global random)"))
