...
phase_1_b2_5 <- 
  rt_summary_data_e2 %>% 
  mutate(epoch = as.factor(epoch)) %>% 
  filter(epoch %in% c(2,5))


phase_1_BF_b2_5 <- 
  anovaBF(meanRT ~ patArrowP1*TT*epoch + subj,
          data= data.frame(phase_1_b2_5),
          whichRandom = "subj")

# does the interaction of TT and cue add anything?
int_effect_b2_5 <- phase_1_BF_b2_5[11]/phase_1_BF_b2_5[8]
...
int_effect_all_exp2 <- all_exp2_BF[4]/all_exp2_BF[3]
# the evidence favours an absence of an interaction effect
...
To provide further support for the absence of the interaction between the factors of configuration type and endogenous cue, the data from across the experiment (epochs 1-10) were analysed with a Bayesian ANOVA with only the factors of configuration and endogenous cue. The best fitting model was that with the two factors and no interaction, `r report_BF_and_error(all_exp2_BF[3])`. 

BF~10~ =`r changeSciNot(extractBF(all_exp2_BF[3])$bf)` &plusmn; `r round(100*extractBF(all_exp2_BF[3])$error,2)`%. 

The addition of the interaction term did not strengthen the model, with support evident for the absence of the interaction, `r report_BF_and_error(int_effect_all_exp2, sci_not = FALSE)`. 

BF~01~ = `r extractBF(int_effect_all_exp2)$bf` &plusmn; `r round(100*extractBF(int_effect_all_exp2)$error,2)`%.
...
The analysis of eye-movements during contextual cuing tasks [@beesley2018; @tseng2004] has revealed a characteristic scanning pattern comprising two phases: firstly, search occurs in a seemingly random manner, as the eyes move between distractors in the central region of the screen, before secondly moving in a more directed manner towards the target position. Contextual cuing is a result of a cessation of the first (random) phase at an earlier timepoint in the trial, such that processing of repeated distractors will, on average, result in fewer fixations. With respect to the curent study, in Experiments 1 and 2 we have initially directed attention towards the side of the screen that contains the target on cued trials. This will bring about an early cessation of the first phase of the search process. From here, however, it seems that eye-movements are still facilitated by the repetition of the context. 
